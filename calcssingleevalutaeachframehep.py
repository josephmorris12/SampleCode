...
# of heparin the calculations will always yield a 1D matrix that is suitable for deccorelation.
# Some of these functions are shared by heparinchaincalcs.py. The ones that are not shared with
# heparinchaincalcs.py have been adjusted to give a 1D matrix by averaging the properties in each
# frame.
...
def dihedrals(traj):
    ...
    -------
    None.
    1D array with all the average dihedrals for every frame.
    ...
    avgdihedrals = np.average(dihedirals, axis=1)
    avgdihedrals = avgdihedrals.flatten()
    return avgdihedrals
...
def bondangle(traj):
    ...
    -------
    None.
    1D array with all the average bond angles for every frame.
    ...
    avganglesarray = np.average(anglesarray, axis=1)
    avganglesarray = avganglesarray.flatten()
    return avganglesarray
...
def bondlengths(traj):
    ...
    -------
    None.
    1D array with the average bond lengths for every frame.
    ...
    # Calc distances between each pair of atoms
    distarray = md.compute_distances(traj, atomarray)
    avgdistarray = np.average(distarray, axis=1)
    avgdistarray = avgdistarray.flatten()
